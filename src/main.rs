...
fn main() {
    ...
    let dns_records = client.get_dns_records()
        .unwrap_or_else(|error| {
            eprintln!("{}", error);
            exit(1);
        });

    let target_dns_records = client.find_record_ids(dns_records, &args.record_name)
        .unwrap_or_else(|error| {
            eprintln!("{}", error);
            exit(1);
        });

    if args.a_record_value != "" {
        let a_record = target_dns_records.iter().find(|record| {
            record["type"].as_str().unwrap() == "A"
        }).unwrap_or_else(|| {
        ...
        _ = client.update_record(a_record_id,&serde_json::Value::String(args.a_record_value))
            .unwrap_or_else(|error| {
                eprintln!("{}", error);
                exit(1);
        });
    ...
    if args.aaaa_record_value != "" {
        let aaaa_record = target_dns_records.iter().find(|record| {
            record["type"].as_str().unwrap() == "AAAA"
        }).unwrap_or_else(|| {
        ...
        _ = client.update_record(aaaa_record_id,&serde_json::Value::String(args.aaaa_record_value))
            .unwrap_or_else(|error| {
                eprintln!("{}", error);
                exit(1);
        })
